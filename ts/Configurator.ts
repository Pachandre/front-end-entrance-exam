...
function ImageInput(state: State<string>, id: string) {
    ...
    if (state.value !== "") preview.src = state.value;
    state.listen((val) =>
        val !== "" ? (preview.src = val) : preview.removeAttribute("src")
    );
    ...
    window.addEventListener("dragleave", (e) => {
        e.preventDefault();
        --drag;
        if (drag === 0) view.classList.toggle("dropping", false);
    });
...
function Listed<T extends Iid>(
    name: string,
    state: State<T[]>,
    itemFactory: () => T,
    elementFactory: (item: T) => [HTMLElement, CleanerFunction?],
    form?: {
        creator: FormCreator<T>;
        saverFactory: (t: T) => FormSaver<T>;
    }
) {
    const list = element("ul", { class: "list" });
    const createElement = (t: T) => {
...
function openDialog<T extends Iid>(
    t: T,
    creator: FormCreator<T>,
    save: (newT: Omit<T, "id">) => void
) {
...
function EditButton<T extends Iid>(
    state: State<T[]>,
    id: string,
    creator: FormCreator<T>
) {
...
const Languages = (state: State<ILanguage[]>) =>
    Listed(
        "languages",
        state,
        () => ({
        ...
        (t) => {
            const select = element(
                ...
                Array.from({ length: 5 }, (_, idx) => {
                    ...
                    if (t.level === idx + 1) opt.selected = true;
                    return opt;
                })
            );
            const input = element("input", {
                ...
                events: {
                    input() {
                        state.update((langs) =>
                            langs.map((l) =>
                                l.id === t.id ? { ...l, name: input.value } : l
                            )
                        );
                    },
                },
...
const ListedEditables = <
    K extends PropertyKey,
    T extends Iid & Record<K, string>
>(
    name: string,
    state: State<T[]>,
    itemFactory: () => T,
    ...
    Listed(
        ...
        {
            creator,
            saverFactory: (t: T) => (newT) => {
                state.update((s) =>
                    s.map((v) => (v.id === t.id ? { ...v, ...newT } : v))
                );
            },
        }
    );

function TextInput(_class: string, value?: string) {
    const el = element("input", {
        class: ["input", _class],
        attrs: { type: "text" },
    });
...
function Titled(title: string, ...children: HTMLElement[]) {
...
function DateView(d: IDateOrRange): [HTMLElement, () => IDateOrRange] {
    ...
    type.value = d.type;
    const from = TextInput(
        "date-from",
        d.type === "date" ? d.value : d.value[0]
    );
...
function TagView(_tags: string[]): [HTMLElement, () => string[]] {
    const input = element("input", {
        class: "input",
    ...
    const tags: Set<string> = new Set(_tags);
...
function ExperienceForm(t: IExperience): Form<IExperience> {
    ...
    const get = () => {
        const experience: Omit<IExperience, "id"> = {
            date: getDate(),
...
const Experience = (state: State<IExperience[]>) =>
    ListedEditables(
        "experience",
        state,
        () => ({
...
function IconsView(_icons: string[]): [HTMLDivElement, () => string[]] {
    const iconsContainer = element("div", { class: "container" });
    const icons: Set<string> = new Set();
    const add = (src: string) => {
        if (icons.has(src)) return;
        icons.add(src);
        iconsContainer.append(
            element("img", { class: "icon", attrs: { src } })
        );
    };
    _icons.forEach(add);
            ...
            change() {
                ...
                for (let i = 0; i < files.length; ++i) {
                    ...
                    if (file.size > MAX_ICON_SIZE) {
                        alert(
                            `Maximum allowed icon size: ${MAX_ICON_SIZE_HUMAN}`
                        );
                        continue;
                    }
    ...
    const get = () =>
        Array.from(
            iconsContainer.querySelectorAll<HTMLImageElement>("img")
        ).map((img) => img.src);
    return [
...
function ToolsetForm(t: IToolset): Form<IToolset> {
    const titleInput = TextInput("title", t.title);
    const title = Titled("Title", titleInput);
...
const Toolsets = (state: State<IToolset[]>) =>
    ListedEditables(
        "toolsets",
        state,
        () => ({
...
function EducationForm(t: IEducation): Form<IEducation> {
    const [date, getDate] = DateView(t.date);
    const titleInput = TextInput("title", t.title);
    const title = Titled("Title", titleInput);
    ...
    const get = () => {
        const education: Omit<IEducation, "id"> = {
            date: getDate(),
            title: titleInput.value,
            tags: getTags(),
...
const Education = (state: State<IEducation[]>) =>
    ListedEditables(
        "education",
        state,
        () => ({
...
export default function Configurator(resume: State<IResume>) {
    const openState = new LocalStorageState<IOpenSections>("open-sections", {
        picture: false,
        about: false,
        languages: false,
        experience: false,
    ...
    return element("aside", { id: "configurator" }, [
        ...
        Section(
            new InnerState(openState, "languages"),
            "languages",
            "Languages",
            [Languages(languages)]
        ),
        Section(
            new InnerState(openState, "experience"),
            "experience",
            "Experience",
        ...
        Section(
            new InnerState(openState, "education"),
            "education",
            "Education",
            [Education(education)]
        ),
