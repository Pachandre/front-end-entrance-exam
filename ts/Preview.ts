...
import State, { effect, LocalStorageState } from "./state";

function toScale(t: number) {
...
function Scaler(state: State<number>) {
    const add = (v: number) => {
...
function Controls(scale: State<number>) {
...
export default function Preview(resume: State<IResume>) {
    const scale = new LocalStorageState<number>("resume-scale", 0);
    ...
    const updateResume = (newResume: IResume) => {
