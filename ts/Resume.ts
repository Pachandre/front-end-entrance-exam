...
function dateToString(date: IDateOrRange) {
    return date.type === "date"
        ? `${date.value}`
        : `${date.value[0]} - ${date.value[1]}`;
}

function Section(id: string, children: Element[]) {
...
function Language(lang: ILanguage) {
...
function Toolset(tool: IToolset) {
    return element("div", { class: "toolset" }, [
        element("h2", { class: "title", text: tool.title }),
        ...tool.icons.map((icn) =>
            element("img", { class: "icon", attrs: { src: icn } })
        ),
    ]);
}

function Experience(exp: IExperience, mostRecent: boolean) {
    ...
    if (mostRecent)
        dateContainer.append(
            element("span", { class: "badge", text: "most recent" })
        );
        ...
        element("div", { class: "text-container" }, [
            ...
            element("div", { class: "right" }, [
                element(
                    "ul",
                    { class: "description" },
                    exp.description.map((line) => element("li", { text: line }))
                ),
            ]),
        ]),
    ]);
    if (mostRecent) el.classList.add("most-recent");
    return el;
}

function Education(edu: IEducation, mostRecent: boolean) {
    const dateContainer = element("div", { class: "date-container" }, [
        element("span", {
    ...
    const el = element("div", { class: "education" }, [
        ...
        element("div", { class: "main-container" }, [
            ...
            element(
                "div",
                { class: "tags" },
                edu.tags.map((tag) => element("span", { text: `#${tag}` }))
            ),
        ]),
...
function Intrests(intrests: string[]) {
    return element(
        "div",
        { class: "intrests" },
        intrests.map((int) => element("span", { class: "intrest", text: int }))
    );
}

export default function Resume(resume: IResume) {
    const picture = element("section", { id: "picture" });
    {
        const img = element("img", { class: "image" });
        if (resume.picture !== "") img.src = resume.picture;
        picture.append(img);
    }
    ...
    const languages = Section("languages", [
        ...
        element(
            "div",
            { class: "container" },
            resume.languages.map((lang) => Language(lang))
        ),
    ]);
    const experience = Section("experience", [
        ...
        element(
            "div",
            { class: "container" },
            resume.experience.map((exp, idx) => Experience(exp, idx === 0))
        ),
    ]);
    const tools = Section("tools", [
        ...
        element(
            "div",
            { class: "container" },
            resume.tools.map((toolset) => Toolset(toolset))
        ),
    ]);
    const education = Section("education", [
        ...
        element(
            "div",
            { class: "container" },
            resume.education.map((edu, idx) => Education(edu, idx === 0))
        ),
    ]);
